$(function () {
    ...
    $.upLoadDefaults.property = {
        ...
        sendurl: "https://wxapi.YiDaBus.com/api/upload/Upload?isThumbnail=true", //
       // sendurl: "http://localhost:9000/api/upload/Upload?isThumbnail=true", //发送地址
            ...
            uploader.on('uploadSuccess', function (file, data) {
                ...
                if (data.code == '200') {
                    ...
                    if (!p.multiple) {
                        ...
                        parentObj.siblings(".upload-path-thumb").val(data.thumb);
                    } else {
                        addImage(parentObj, data.path, data.thumb, data.name, data.ext);
                    }
                    var progressObj = parentObj.children(".upload-progress");
...
=====================================================*/
//添加图片相册
function addImage(targetObj, originalSrc, thumbSrc, fileName, ext) {
    //插入到相册UL里面
    if (ext == "mp4") {
        var newLi = $('<li>'
    ...
    + '<video src="' + originalSrc + '" bigsrc="' + originalSrc + '"style="width: 106px; height: 106px;" poster="/Content/img/video.png"/>'
...
function delImg(obj) {
    ...
    // 删除文件 
    // alert($(obj).siblings(".img-box").children("img").attr("src")); return;
    debugger;
    $.post("/housemanage/housepics/DeleteFormFile", { keyvalue: $(obj).siblings(".img-box").children("img").attr("src") }, function (results) {});
}
//下载文件
