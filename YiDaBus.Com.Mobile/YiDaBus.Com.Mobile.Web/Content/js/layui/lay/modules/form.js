    ...
    var submit = function () {
        ...
        layui.each(verifyElem, function (_, item) {
            ...
            if (verify[ver] && (isFn ? tips = verify[ver](value, item) : !verify[ver][0].test(value))) {
                //var verifyMsg = ver == "required" && $(item).attr("lay-reqmsg") != "" ? $(item).attr("lay-reqmsg") : verify[ver][1];
                var verifyMsg = '';
                if ($(item).attr("lay-reqmsg") != "" && value == '') {
                    verifyMsg = $(item).attr("lay-reqmsg");
                } else {
                    verifyMsg = verify[ver][1];
                }

                form.config.errorMsgCall(tips || verifyMsg);
                //非移动设备自动定位焦点
