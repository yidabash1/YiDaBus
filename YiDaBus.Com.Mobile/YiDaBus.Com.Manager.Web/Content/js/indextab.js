(function ($) {
    $.nfinetab = {
            ...
            $('.menuTab').each(function () {
                if ($(this).data('id') == dataUrl) {
                    if (!$(this).hasClass('active')) {
                        $(this).addClass('active').siblings('.menuTab').removeClass('active');
                        $.nfinetab.scrollToTab(this);
                        $('.mainContent .NFine_iframe').each(function (index, obj) {
                            if ($(this).data('id') == dataUrl) {
                                $(this).attr('src', $(obj).attr('data-id')).show().siblings('.NFine_iframe').hide();
                                return false;
                            }
                        });
                    }
        ...
        addTabByUrl: function (url) {
            var $this;
            top.$('.menuItem').each(function (index, obj) {
                if ($(this).attr('href').toLowerCase() == url.toLowerCase().split('?')[0]) {
                    $this = $(this);
                    return true;
                }
            });
            ...
            top.$('.menuTab').each(function () {
                if ($(this).data('id') == dataUrl) {
                    if (!$(this).hasClass('active')) {
                        ...
                        $.nfinetab.scrollToTab(this);
                        top.$('.mainContent .NFine_iframe').each(function (index, obj) {
                            if ($(this).data('id') == dataUrl) {
                                $(this).attr('src', url).show().siblings('.NFine_iframe').hide();
                                return false;
                            }
                        });
                    } else {
            ...
            if (flag) {
                var str = '<a href="javascript:;" class="active menuTab" data-id="' + dataUrl + '">' + menuName + ' <i class="fa fa-remove"></i></a>';
                var str1 = '<iframe class="NFine_iframe" id="iframe' + dataId + '" name="iframe' + dataId + '"  width="100%" height="100%" src="' + url + '" frameborder="0" data-id="' + dataUrl + '" seamless></iframe>';
        ...
        addTabByUrlNoMenu: function (url,name) {
            
            var str = '<a href="javascript:;" class="active menuTab" data-id="' + url + '">' + name + ' <i class="fa fa-remove"></i></a>';
            $('.menuTab').removeClass('active');
            $('.NFine_iframe').hide()
            var str1 = '<iframe class="NFine_iframe" id="iframe' + 00000000 + '" name="iframe' + 00000000 + '"  width="100%" height="100%" src="' + url + '" frameborder="0" data-id="' + url + '" seamless></iframe>';
            top.$('.mainContent').find('iframe.NFine_iframe').hide();
            top.$('.mainContent').append(str1);
            $.loading(true);
            $('.mainContent iframe:visible').load(function () {
